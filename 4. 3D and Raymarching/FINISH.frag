...
intersect rayMarch(vec3 rayOrigin, vec3 rayDirection) {
  ...
  for (int i = 0; i < 128; i++) {
    ...
    if (MAX_BOUNCES == 0 || SCENE == 0) {
      dist += scene;
    } else {
      // Reduce the influence of reflected distances
      dist += scene * mix(1.0, 0.1, float(bounces) / float(MAX_BOUNCES));

      // Reflect the ray off of an object surface
      if (scene < EPSILON && MAX_BOUNCES > 0 && bounces < MAX_BOUNCES) {
        bounces++;
        vec3 normal = calcNormal(position);
        rayDirection = mix(rayDirection, reflect(rayDirection, normal), 0.05);
      }
    }
...
void main() {
  // Transformation matrices make it easy to point the camera at a given point,
  // in this case determined by a mouse drag interaction.
  vec3 cameraTarget = vec3(u_mouse.x * -2.0 + 1.0, u_mouse.y * 2.0 - 1.0, 1.0);
  cameraMatrix = initCameraMatrix(CAMERA_POSITION, cameraTarget, 0.0);
